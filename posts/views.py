...
def get_posts(request):
    ...
    posts = Post.objects.filter(publisted_date__lte=timezone.now()).order_by('published_date')
...
def post_detail(request, pk):
    ...
    Or retutn a 404 error if the Post is not found"""
...
def create_or_edit_post(request, pk=None):
    """Create a vieq that allows us to create or edit a post depending if the post ID is null or not"""
